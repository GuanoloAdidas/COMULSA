...
---------------------------------------------------*/	
	...
	function FirstLoad() {	
		...
		//aqui cambie el color del background fondo de pantalla color cambie cambio
		
		$("html,body").animate({scrollTop: 0}, 1);
		
		if ($("#page-content").hasClass("light-content")) {
			$("main, nav").css('background-color', '#eeeeee');
			$('#magic-cursor').addClass('light-content');
			if( $('#hero').length > 0 ){						
				if( $('#hero').hasClass("has-image")) {	
		...
		$('a.ajax-link').on('click', function() {
			...
			TweenMax.to('#ball', 0.3,{borderWidth:"2px",scale:1,backgroundColor:"rgba(0, 0, 0, 1)",opacity:1});
